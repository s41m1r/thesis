...
\begin{itemize}
	...
	\item{\bfseries User evaluation.}
		...
		Following research on visualization, the \emph{reflect} stage is clarified by the help of \gls{nbgm} \citep{DBLP:journals/ivs/MeyerSQM15}. \Gls{nbgm} provides a framework for reflecting upon the design decisions and their implications for the final user. For this thesis, the \gls{nbgm} can be used in two ways. First, it can help at combining together the artifacts as an overall diagram by specifying the blocks each of the artifacts A1 -- A4 tackle and connecting these blocks via guidelines, where each guideline would capture interactions among blocks. In this way, not only the contributions become clear, but also a big-picture blueprint is created which can be presented to the final user (e.g., a project manager) who may clearly see the contribution in context. Second, the \gls{nbgm} will help the project manager to compare the different blocks between the techniques presented in this study and other existing approaches. This would highlight, for instance, the unique contributions of this thesis in contexts which are out of the reach of existing techniques. \\
		\end{sloppypar}

	Finally, the context of this research is software development in organisations. Therefore the \gls{nbgm} shall target project managers or similar roles who need to gather monitoring-related knowledge about the process. Their input can be collected with the help of various user-study techniques, such as questionnaires and interviews. For instance, an applicable model is the \citep{DBLP:journals/misq/Davis89,DBLP:journals/iam/LegrisIC03,DBLP:journals/misq/VenkateshMDD03}. Based on the outcomes of this evaluation, new features may be developed or existing features improved. This would then restart the design cycle with renewed goals. \\
